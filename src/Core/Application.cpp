...
namespace blueberry
{
	...
	void Application::createEngine()
	{
		...
		engine_.frames = TypeList<EngineFrame>(EngineFrame(), BLUEBERRY_MAX_FRAMES_IN_FLIGHT);
		...
		for (int i = 0; i < BLUEBERRY_MAX_FRAMES_IN_FLIGHT; i++)
		{
			...
			if (vkCreateFence(Application::logicalDevice_.device, &fenceInfo, nullptr, &engine_.frames[i].inFlightFence) != VK_SUCCESS)
			{
				throw - 1;
			}
			...
			engine_.frames[i].uniformBuffer = createBuffer(logicalDevice_.device, sizeof(UBO), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
			engine_.frames[i].uniformBufferMemory = createDeviceMemory(physicalDevice_.device, logicalDevice_.device, engine_.frames[i].uniformBuffer, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

			vkBindBufferMemory(logicalDevice_.device, engine_.frames[i].uniformBuffer, engine_.frames[i].uniformBufferMemory, 0);
			vkMapMemory(logicalDevice_.device, engine_.frames[i].uniformBufferMemory, 0, sizeof(UBO), 0, &engine_.frames[i].uniformBufferMap);

			bufferInfo.offset = 0;
			bufferInfo.buffer = engine_.frames[i].uniformBuffer;
	...
	void Application::draw()
	{
		...
		TypeList<TypeList<uint32_t>> textures;
		TypeList<TypeList<Transform>> transforms;

		for (int i = 0; i < Entity::getComponentPool<Sprite>().components.size(); i++)
		{
			Sprite sprite = Entity::getComponentPool<Sprite>().components[i];
			textures[sprite.windowIndex_].add((sprite.textureGeneration_ == Texture::generations_[sprite.textureIndex_]) ? sprite.textureIndex_ : 0);
			transforms[sprite.windowIndex_].add(Entity::getComponentPool<Transform>().components[i]);
		}

		vkWaitForFences(logicalDevice_.device, 1, &engine_.frames[currentFrame_].inFlightFence, VK_TRUE, UINT64_MAX);
		vkResetFences(logicalDevice_.device, 1, &engine_.frames[currentFrame_].inFlightFence);
		...
		/*if (transformBufferSize > engine_.frames[currentFrame_].transformBufferSize)
		{
			engine_.frames[currentFrame_].transformBufferSize = transformBufferSize;

			recreateBuffer(physicalDevice_.device, logicalDevice_.device, engine_.frames[currentFrame_].transformBuffer, engine_.frames[currentFrame_].transformBufferMemory,
			...
			vkBindBufferMemory(logicalDevice_.device, engine_.frames[currentFrame_].transformBuffer, engine_.frames[currentFrame_].transformBufferMemory, 0);

			updateDescriptorSets(logicalDevice_.device, engine_.descriptorSets[currentFrame_], engine_.frames[currentFrame_].transformBuffer, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
		}

		if (spriteBufferSize > engine_.frames[currentFrame_].spriteBufferSize)
		{
			engine_.frames[currentFrame_].spriteBufferSize = spriteBufferSize;

			recreateBuffer(physicalDevice_.device, logicalDevice_.device, engine_.frames[currentFrame_].spriteBuffer, engine_.frames[currentFrame_].spriteBufferMemory,
				VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, spriteBufferSize);

			vkBindBufferMemory(logicalDevice_.device, engine_.frames[currentFrame_].spriteBuffer, engine_.frames[currentFrame_].spriteBufferMemory, 0);

			updateDescriptorSets(logicalDevice_.device, engine_.descriptorSets[currentFrame_], engine_.frames[currentFrame_].spriteBuffer, 2, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
		}
		...
		if (transformBufferSize > 0)
		{
			...
			copyBuffer(logicalDevice_.device, logicalDevice_.graphicsQueue, engine_.commandPool, engine_.stagingBuffer, engine_.frames[currentFrame_].transformBuffer, transformBufferSize);
		}
		...
		if (spriteBufferSize > 0)
		{
			...
			copyBuffer(logicalDevice_.device, logicalDevice_.graphicsQueue, engine_.commandPool, engine_.stagingBuffer, engine_.frames[currentFrame_].spriteBuffer, spriteBufferSize);
		}*/

		// Allocation des command buffers

		if (engine_.frames[currentFrame_].commandBuffers.size() < Window::windows_.size()) // augmente le nombre de command buffer si nécessaire afin de submit les draw call en même temps
		{
			int commandBufferSize = engine_.frames[currentFrame_].commandBuffers.size();
			engine_.frames[currentFrame_].commandBuffers.resize(Window::windows_.size());

			VkCommandBufferAllocateInfo allocInfo{};
			allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
			allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
			allocInfo.commandPool = Application::engine_.commandPool;
			allocInfo.commandBufferCount = Window::windows_.size() - commandBufferSize;

			if (vkAllocateCommandBuffers(Application::logicalDevice_.device, &allocInfo, &engine_.frames[currentFrame_].commandBuffers[commandBufferSize]) != VK_SUCCESS)
			{
				throw - 1;
			}
		}
		...
		for (int i = 0; i < Window::windows_.size(); i++)
		{
			...
			VkCommandBuffer commandBuffer = engine_.frames[currentFrame_].commandBuffers[i];
			...
			renderPassInfo.pClearValues = &Window::windows_[i].clearColor;
			...
			{
				switch (Pipeline::pipelines_[j].type)
				{
				case GUI:
				{
				}
				...
				case SPRITE:
				{
					...
					memcpy(engine_.frames[currentFrame_].uniformBufferMap, &ubo, sizeof(ubo));
		...
		submitInfo.commandBufferCount = engine_.frames[currentFrame_].commandBuffers.size();
		submitInfo.pCommandBuffers = engine_.frames[currentFrame_].commandBuffers.data();

		submitInfo.signalSemaphoreCount = renderFinishedSemaphores.size();
		submitInfo.pSignalSemaphores = renderFinishedSemaphores.data();

		if (vkQueueSubmit(logicalDevice_.graphicsQueue, 1, &submitInfo, engine_.frames[currentFrame_].inFlightFence) != VK_SUCCESS)
		{
			throw - 1;
		}
    ...
    void Application::run(ApplicationListener* applicationListener)
    {
